...
export default function GenreFilterUI() {
...
document.title = "Flickster | Movies By Genre"  
  useEffect(() => {
  ...
  const fetchMovies = async (genreId, currentPage = page) => {
    setLoading(true);
    try {
      const { data } = await axios.get(`/discover/movie?with_genres=${genreId}&page=${currentPage}`);
